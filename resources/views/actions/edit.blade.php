<x-app-layout>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-xl text-gray-900 leading-tight">
                        {{ __('Edit Remote Action') }}
                    </h2>
                    <p class="text-xs text-gray-500 mt-0.5">
                        {{ __('Perbarui nama, ikon, perintah SSH, dan perangkat target') }}
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="py-8" x-data="editActionForm({{ json_encode($computersJson) }}, {{ json_encode($selectedIdsJson) }})">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl border border-gray-200/80 shadow-xs p-8">

                <form method="POST" action="{{ route('actions.update', $action) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <textarea id="command" name="command" rows="3" required
                                    ...
                                    placeholder="DISPLAY=:0 xdotool key F5">{{ old('command', $action->command) }}</textarea>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('editActionForm', (allDevices, initialSelectedIds) => ({
