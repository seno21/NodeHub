            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">
                        {{ __('Automate SSH commands across target devices') }}
                    </p>
                </div>
            </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider">
                        {{ __('Remote Actions') }} (<span x-text="actions.length"></span>)
                    </h3>
                </div>
                            ...
                            <div>
                                <div class="flex items-start justify-between gap-3 mb-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <span class="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-500 mt-0.5">
                                                ...
                                                <span x-text="act.computers_count + ' Target Devices'"></span>
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <a :href="'/actions/' + act.id"
                                        class="p-2 rounded-xl text-gray-400 hover:text-gray-700 hover:bg-slate-100 transition" title="Settings & Details">
                                ...
                                <p class="text-xs text-gray-600 line-clamp-2 leading-relaxed mb-3" x-text="act.description || 'No description provided'"></p>
                ...
                <div class="bg-white rounded-2xl border border-gray-200/80 p-8 text-center" x-show="actions.length === 0">
                    ...
                    <h3 class="font-bold text-base text-gray-900 mb-1">No Remote Actions</h3>
                    <p class="text-xs text-gray-500 max-w-md mx-auto mb-4">
                        Create remote actions to execute SSH commands like browser refresh, app restart, or device reboot automatically.
                    </p>
                    <a href="{{ route('actions.create') }}"
            ...
            <div class="bg-[#0d1117] rounded-2xl border border-slate-800 shadow-2xl overflow-hidden">
                ...
                <div id="terminal-container" class="p-4 h-96 sm:h-[450px] overflow-y-auto font-mono text-xs leading-6 space-y-1 bg-[#0d1117]">
                    ...
                    <p x-show="logs.length === 0" class="text-gray-600 italic py-6 text-center">
                        No action executed yet. Click "Exec" on any action card above.
                    </p>
                </div>
            </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('remoteActionDashboard', (actions) => ({
                ...
                async executeRemoteAction(act) {
                    ...
                    this.addLog(`=== STARTING ACTION: "${act.name}" ===`, 'text-cyan-400 font-bold');
                    this.addLog(`[STEP 1/2] Checking SSH connection to ${act.computers_count} target devices...`, 'text-blue-400 font-bold');
